  ...
  if (millis() - tmr0 > 25)
  {
    ...
    if (psize > 0)
    {
      ...
      else if (ints[0] == 2)
      {
        ...
        //теперь у всех движков постоянные мощности
        rot_val = (rot_val<0)? -50 : ((rot_val>0)? 50 : 0);
        win_val = (win_val<0)? -255 : ((win_val>0)? 255 : 0);
        arm_val = (arm_val<0)? -100 : ((arm_val>0)? 100 : 0);
